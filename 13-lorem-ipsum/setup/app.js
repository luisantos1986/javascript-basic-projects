...
//Math.random() nos da 0 a 0.9
//Math.floor() redondea el numero 

/* function getRandomText(){
     const numero = Math.floor(Math.random() * text.length) 
     return text[numero]
} */

/* let limite = 4
form.addEventListener(EVENTS.CLICK, function(e) {
    e.preventDefault()
    let texto = ""
    for (let i = 0; i < amount.value; i++) {
        texto = `${texto}<p>${getRandomText()}</p> <br/>`
  }
...
form.addEventListener(EVENTS.SUBMIT, function (e) {
   ...
   const random = Math.floor(Math.random() * text.lenth);
